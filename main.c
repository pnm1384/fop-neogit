...
int copied_file_array_lines[2000], arr11[2000], arr22[2000];
char for_use_1[2000][2000], for_use_2[2000][2000];
char commit_files_1[2000][2000], commit_files_2[2000][2000];
int commit_ok_1[2000], commit_ok_2[2000];
int for_use_1_lines[2000], for_use_2_lines[2000];
char Month_names[12][4] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};
int copied_file_array_ok[2000], array_just_for_use[2000];
char temppp[1000];
...
int line_is_null(char *ss)
{
	int nnnn = strlen(ss);
	for (int i = 0; i < nnnn; i++)
	{
		if (!is_white(ss[i])) return 0;
	}
	return 1;
}

int version_of_file_2(char *name1, char *name2)
{
	int indx = -1;
	int nnn = strlen(name1);
	...
	name2[indx2] = '\0';
	int is_ok = 0;
	if (strcmp(name1, name2) == 0) is_ok = 1;
	name1[indx] = '#';
	name2[indx2] = '#';
	return is_ok;
}

void get_add_from_number_name(char *ss, char *name)
{
	int ind = -1;
	int nnn = strlen(name);
	for (int i = nnn - 1; i >= 0; i--)
...
void get_file_name_bad(char *add, char *ss) // only correct address for real file !
{
	int indx = -1;
	int nnn = strlen(add);
	for (int i = nnn - 1; i >= 0; i--)
...
void add_nowadd_to_add(char *add) // shart : strlen(add) > 0
{
	if (add[0] == '/') return;
	char now_add[1000];
...
int get_last_commit_of_branch(char *name)
{
	get_repo_add();
	char tmpp[1000];
...
void get_commit_files_add(char *ss, int com_id)
{
	get_repo_add();
	char bad_add[1000];
	strcpy(bad_add, repo_add);
	strcat(bad_add, "/.neogit/commits/");
	int xxx = com_id;
	int nnnn = strlen(bad_add);
	if (xxx == 0) bad_add[nnnn++] = '0';
	while (xxx > 0)
	{
		bad_add[nnnn++] = xxx % 10 + '0';
		xxx /= 10;
	}
	bad_add[nnnn] = '\0';
	strcat(bad_add, "/files");
	strcpy(ss, bad_add);
	return;
...
void run_init(int argc, char *argv[])
{
	...
	fclose(ptr);
	ptr = fopen(".neogit/start_branch", "w");
	fprintf(ptr, "%d", -1);
	fclose(ptr);
	ptr = fopen(".neogit/end_branch", "w");
	fprintf(ptr, "%d", -1);
	fclose(ptr);
	return;
}
...
void run_branch(int argc, char *argv[])
{
	...
	*/
	...
	fclose(ptr);
	strcpy(tmpp, repo_add);
	strcat(tmpp, "/.neogit/start_branch");
	ptr = fopen(tmpp, "w");
	fprintf(ptr, "%d", ffatt);
	fclose(ptr);
	return;
...
int get_balance_braces(char *add)
{
	char tmp_for[1000];
	get_file_format_bad(tmp_for, add);
	if (strcmp(tmp_for, ".cpp") != 0 && strcmp(tmp_for, ".c") != 0 && strcmp(tmp_for, ".txt") != 0)
	...
	if ((para == 0) && (kro == 0) && (bra == 0)) return 1;
	return 0;
}

int get_format_check(char *add)
{
	char tmp_for[1000];
	get_file_format_bad(tmp_for, add);
	if (strcmp(tmp_for, ".cpp") != 0 && strcmp(tmp_for, ".c") != 0 && strcmp(tmp_for, ".mp4") != 0 && strcmp(tmp_for, ".wav") != 0 && strcmp(tmp_for, ".mp3") != 0 && strcmp(tmp_for, ".txt") != 0)
	{
		return -1;
	}
	return 1;
	...
	if (argc == 2)
	{
		...
		while (fgets(ssss, sizeof(ssss), ptr_staging) != NULL)
		{
			...
			while (fgets(prec_com, sizeof(prec_com), ptr_prec) != NULL)
			{
				...
				if (strcmp(prec_com, "balance-braces") == 0) ans = get_balance_braces(bad_add);
				if (strcmp(prec_com, "format-check") == 0) ans = get_format_check(bad_add);
				printf("%s", prec_com);
				int bbb = 0;
				if (ans == -1) bbb = 1;
...
void run_grep_on_one_file(char *add, char *wild, int have_line)
{
	FILE *ptr = fopen(add, "r");
	char ssss[1000], ssss2[1000];
	int cnt_line = 0;
	while (fgets(ssss, sizeof(ssss), ptr) != NULL)
	{
		cnt_line++;
		...
		strcpy(ssss2, ssss);
		int nnnn = strlen(ssss);
		for (int i = 0; i < nnnn; i++) array_just_for_use[i] = 0;
		for (int i = 0; i < nnnn; i++)
		{
			if (is_white(ssss[i])) ssss[i] = ' ';
		}
		...
		for (int i = 0; i <= nnnn; i++)
		{
			...
			if (ssss[i] == ' ' || ssss[i] == '\0')
			{
				if (las != -1)
				{
					// return ssss[i] !
					ssss[i] = '\0';
					if (name_equal_to_wild(ssss + las, wild))
					{
						for (int j = las; j < i; j++)
						{
							array_just_for_use[j] = 1;
							have_one = true;
						}
					}
					if (i != nnnn) ssss[i] = ' ';
		...
		if (have_line) printf("line %d: ", cnt_line);
		for (int i = 0; i < nnnn; i++)
		{
			if (array_just_for_use[i]) printf("\033[1;36m%c\033[1;0m", ssss2[i]);
			else printf("%c", ssss2[i]);
		}
		printf("\n");
	}
	fclose(ptr);
...
void run_grep(int argc, char *argv[])
{
	...
	add_nowadd_to_add(argv[3]);
	char bad_add[1000];
	strcpy(bad_add, argv[3]);
	change_add_to_name(bad_add);
	int cnt_bad = get_number_of_versions(bad_add);
	char tmpp[1000];
	strcpy(tmpp, repo_add);
	strcat(tmpp, "/.neogit/commits/");
	int com_id = -1;
	...
	if (com_id == -1)
	{
		...
		run_grep_on_one_file(argv[3], argv[5], have_line);
		return;
	}
	int xxx = com_id;
	int nnnn = strlen(tmpp);
	...
	{
		remove_the_enter(ssss);
		if (version_of_file(ssss, bad_add, cnt_bad + 3) != -1)
		{
			have_it = true;
			break;
		}
	...
	if (!have_it)
	{
		printf("this commit does not include this file!\n");
		return;
	}
	char besyar_add[1000];
	strcpy(besyar_add, repo_add);
	strcat(besyar_add, "/.neogit/");
	strcat(besyar_add, ssss);
	run_grep_on_one_file(besyar_add, argv[5], have_line);
	return;
}

void get_good_lines_of_file(char *add, int bgn_lin, int end_lin)
{
	if (bgn_lin == -1 || end_lin == -1)
	{
		bgn_lin = 1;
		end_lin = get_number_of_lines(add);
	}
	size_of_copied_file_array = 0;
	char ssss[1000];
	FILE *ptr = fopen(add, "r");
	for (int i = 0; i < bgn_lin - 1; i++) fgets(ssss, sizeof(ssss), ptr);
	for (int i = bgn_lin; i <= end_lin; i++)
	{
		fgets(ssss, sizeof(ssss), ptr);
		remove_the_enter(ssss);
		if (!line_is_null(ssss))
		{
			int ind = size_of_copied_file_array;
			size_of_copied_file_array++;
			strcpy(copied_file_array[ind], ssss);
			copied_file_array_lines[ind] = i;
		}
	}
	fclose(ptr);
	return;
...
int get_diff_on_two_files(char *add1, char *add2)
{
	int bgn1 = 1, bgn2 = 1;
	int end1 = get_number_of_lines(add1);
	int end2 = get_number_of_lines(add2);
	get_good_lines_of_file(add1, bgn1, end1);
	int nn1 = size_of_copied_file_array;
	for (int i = 0; i < size_of_copied_file_array; i++)
	{
		strcpy(for_use_1[i], copied_file_array[i]);
		for_use_1_lines[i] = copied_file_array_lines[i];
	}
	get_good_lines_of_file(add2, bgn2, end2);
	int nn2 = size_of_copied_file_array;
	for (int i = 0; i < size_of_copied_file_array; i++)
	{
		strcpy(for_use_2[i], copied_file_array[i]);
		for_use_2_lines[i] = copied_file_array_lines[i];
	}
	while (nn1 < nn2)
	{
		for_use_1[nn1][0] = '\0';
		for_use_1_lines[nn1] = -1;
		nn1++;
	}
	while (nn2 < nn1)
	{
		for_use_2[nn2][0] = '\0';
		for_use_2_lines[nn2] = -1;
		nn2++;
	}
	bool is_ok = true;
	for (int i = 0; i < nn1; i++)
	{
		if (strcmp(for_use_1[i], for_use_2[i]) != 0)
		{
			is_ok = false;
			break;
		}
...
void run_diff_on_two_files(char *add1, char *add2, int com_id1, int com_id2)
{
	int bgn1 = 1, bgn2 = 1;
	int end1 = get_number_of_lines(add1);
	int end2 = get_number_of_lines(add2);
	...
	for (int i = 0; i < nn1; i++)
	{
		if (strcmp(for_use_1[i], for_use_2[i]) == 0) continue;
		printf("<<<<<\n");
		if (for_use_1_lines[i] != -1) printf("commit %d - line %d\n", com_id1, for_use_1_lines[i]);
		else printf("commit %d - no number\n", com_id1);
		printf("\033[1;33m%s\033[1;0m\n", for_use_1[i]);
		if (for_use_2_lines[i] != -1) printf("commit %d - line %d\n", com_id2, for_use_2_lines[i]);
		else printf("commit %d - no number\n", com_id2);
...
void run_diff_on_two_files_2(char *add1, char *add2, char *abb_add, char *bra1, char *bra2)
{
	int bgn1 = 1, bgn2 = 1;
	int end1 = get_number_of_lines(add1);
	int end2 = get_number_of_lines(add2);
	...
	for (int i = 0; i < nn1; i++)
	{
		if (strcmp(for_use_1[i], for_use_2[i]) == 0) continue;
		printf("<<<<<%s>>>>>\n", abb_add);
		if (for_use_1_lines[i] != -1) printf("%s - line %d\n", bra1, for_use_1_lines[i]);
		else printf("%s - no number\n", bra1);
		printf("\033[1;33m%s\033[1;0m\n", for_use_1[i]);
		printf("<<<<<%s>>>>>\n", abb_add);
		if (for_use_2_lines[i] != -1) printf("%s - line %d\n", bra2, for_use_2_lines[i]);
		else printf("%s - no number\n", bra2);
		printf("\033[1;34m%s\033[1;0m\n", for_use_2[i]);
	}
	return;
...
void run_diff(int argc, char *argv[])
{
	...
	{
		add_nowadd_to_add(argv[3]);
		add_nowadd_to_add(argv[4]);
		int bgn1 = -1, end1 = -1, bgn2 = -1, end2 = -1;
		int ind = 5;
		while (ind < argc)
			{
			if (strcmp(argv[ind], "-line1") == 0)
			{
				sscanf(argv[ind + 1], "<%d-%d>", &bgn1, &end1);
				ind += 2;
				continue;
			}
			if (strcmp(argv[ind], "-line2") == 0)
			{
				sscanf(argv[ind + 1], "<%d-%d>", &bgn2, &end2);
				ind += 2;
				continue;
			}
		}
		...
		if (end1 > get_number_of_lines(argv[3])) end1 = get_number_of_lines(argv[3]);
		if (end2 > get_number_of_lines(argv[4])) end2 = get_number_of_lines(argv[4]);
		get_good_lines_of_file(argv[3], bgn1, end1);
		int nn1 = size_of_copied_file_array;
		for (int i = 0; i < size_of_copied_file_array; i++)
		{
			strcpy(for_use_1[i], copied_file_array[i]);
		...
		get_good_lines_of_file(argv[4], bgn2, end2);
		int nn2 = size_of_copied_file_array;
		for (int i = 0; i < size_of_copied_file_array; i++)
		{
			strcpy(for_use_2[i], copied_file_array[i]);
		...
		char what_name_1[1000], what_name_2[1000];
		get_file_name_bad(argv[3], what_name_1); get_file_name_bad(argv[4], what_name_2);
		for (int i = 0; i < nn1; i++)
		{
			if (strcmp(for_use_1[i], for_use_2[i]) == 0) continue;
			printf("<<<<<\n");
			if (for_use_1_lines[i] != -1) printf("%s - line %d\n", what_name_1, for_use_1_lines[i]);
			else printf("%s - no number\n", what_name_1);
			printf("\033[1;33m%s\033[1;0m\n", for_use_1[i]);
			if (for_use_2_lines[i] != -1) printf("%s - line %d\n", what_name_2, for_use_2_lines[i]);
			else printf("%s - no number\n", what_name_2);
			printf("\033[1;34m%s\033[1;0m\n", for_use_2[i]);
			printf(">>>>>\n");
		}
	...
	int com_id1 = -1, com_id2 = -1;
	sscanf(argv[3], "%d", &com_id1);
	sscanf(argv[4], "%d", &com_id2);
	char bad_add1[1000], bad_add2[1000];
	get_commit_files_add(bad_add1, com_id1);
	get_commit_files_add(bad_add2, com_id2);
	copy_file_into_array(bad_add1);
	int nn1 = size_of_copied_file_array;
	for (int i = 0; i < nn1; i++) commit_ok_1[i] = 0;
	for (int i = 0; i < nn1; i++)
	{
		strcpy(commit_files_1[i], copied_file_array[i]);
	}
	copy_file_into_array(bad_add2);
	int nn2 = size_of_copied_file_array;
	for (int i = 0; i < nn2; i++) commit_ok_2[i] = 0;
	for (int i = 0; i < nn2; i++)
	{
		strcpy(commit_files_2[i], copied_file_array[i]);
	}
	char badbad[1000], badbad1[1000], badbad2[1000];
	for (int i = 0; i < nn1; i++)
	{
		int have_it = 0;
		for (int j = 0; j < nn2; j++)
		{
			if (version_of_file_2(commit_files_1[i], commit_files_2[j]))
			{
				commit_ok_1[i] = 1;
				commit_ok_2[j] = 1;
				have_it = 1;
				get_add_from_number_name(badbad, commit_files_1[i]);
				printf("%s:\n", badbad);
				strcpy(badbad1, repo_add);
				strcat(badbad1, "/.neogit/");
				strcat(badbad1, commit_files_1[i]);
				strcpy(badbad2, repo_add);
				strcat(badbad2, "/.neogit/");
				strcat(badbad2, commit_files_2[j]);
				if (!get_diff_on_two_files(badbad1, badbad2)) printf("no difference!\n");
				else run_diff_on_two_files(badbad1, badbad2, com_id1, com_id2);
				break;
			}
			if (have_it) break;
		}
		// check conflict in merge !
	}
	char sss[1000], bad2[1000];
	for (int i = 0; i < nn1; i++)
	{
		if (commit_ok_1[i]) continue;
		strcpy(bad2, commit_files_1[i]);
		get_add_from_number_name(sss, bad2);
		printf("%s OK\n", sss);
	}
	for (int i = 0; i < nn2; i++)
	{
		if (commit_ok_2[i]) continue;
		strcpy(bad2, commit_files_2[i]);
		get_add_from_number_name(sss, bad2);
		printf("%s OK\n", sss);
	}
	return;
...
void run_merge(int argc, char *argv[])
{
	...
	int com_id1 = get_last_commit_of_branch(argv[3]);
	int com_id2 = get_last_commit_of_branch(argv[4]);
	char bad_add1[1000], bad_add2[1000];
	get_commit_files_add(bad_add1, com_id1);
	get_commit_files_add(bad_add2, com_id2);
	copy_file_into_array(bad_add1);
	...
	for (int i = 0; i < nn1; i++)
	{
		...
		for (int j = 0; j < nn2; j++)
		{
			if (version_of_file_2(commit_files_1[i], commit_files_2[j]))
			{
				...
				strcat(badbad2, "/.neogit/");
				strcat(badbad2, commit_files_2[j]);
				if (get_diff_on_two_files(badbad1, badbad2))
				{
					have_conflict = 1;
				}
				break;
	...
	for (int i = 0; i < nn1; i++)
	{
		...
		for (int j = 0; j < nn2; j++)
		{
			if (version_of_file_2(commit_files_1[i], commit_files_2[j]))
			{
				...
				if (get_diff_on_two_files(badbad1, badbad2))
				{
					run_diff_on_two_files_2(badbad1, badbad2, badbad, argv[3], argv[4]);
				}
				break;
			}
			if (have_it) break;
	...
	for (int i = 0; i < nn2; i++)
	{
		...
		get_author_email(what_email);
		get_current_branch(what_branch);
		if (strcmp(what_branch, argv[3]) != 0 && strcmp(what_branch, argv[4]) != 0)
		{
			printf("enter the branch first!\n");
			return;
		}
		int par_commit = get_last_commit_id();
		/*
	...
	//	printf("commit message: %s\n", argv[3]);
		...
		strcpy(bad_file, repo_add);
		strcat(bad_file, "/.neogit/end_branch");
		ptr_fat = fopen(bad_file, "w");
		fprintf(ptr_fat, "%d", commit_id);
		fclose(ptr_fat);
	return;
}
...
{
	get_repo_add();
	char fir_branch[1000];
	get_commit_info_line(fir_branch, 1, 2);
	char tmp_add[1000];
	strcpy(tmp_add, repo_add);
	strcat(tmp_add, "/.neogit/branches/");
	strcat(tmp_add, fir_branch);
	strcat(tmp_add, "/commits");
	copy_file_into_array(tmp_add);
	int nn1 = 0, nn2 = 0;
	for (int i = 0; i < size_of_copied_file_array; i++) sscanf(copied_file_array[i], "%d", &arr11[nn1++]);
	int start_bra = -1, end_bra = -1;
	strcpy(tmp_add, repo_add);
	strcat(tmp_add, "/.neogit/start_branch");
	FILE *ptr = fopen(tmp_add, "r");
	char ssss[1000];
	fgets(ssss, sizeof(ssss), ptr);
	sscanf(ssss, "%d", &start_bra);
	fclose(ptr);
	strcpy(tmp_add, repo_add);
	strcat(tmp_add, "/.neogit/end_branch");
	ptr = fopen(tmp_add, "r");
	ssss[1000];
	fgets(ssss, sizeof(ssss), ptr);
	remove_the_enter(ssss);
	sscanf(ssss, "%d", &end_bra);
	fclose(ptr);
	if (start_bra == -1)
	{
		for (int i = 0; i < nn1; i++) 
	...
	strcpy(tmp_add, repo_add);
	strcat(tmp_add, "/.neogit/branch_names");
	ptr = fopen(tmp_add, "r");
	char second_bra[1000];
	while (fgets(ssss, sizeof(ssss), ptr) != NULL)
	{
		remove_the_enter(ssss);
		if (strcmp(ssss, fir_branch) != 0)
		{
			strcpy(second_bra, ssss);
			break;
		}
	}
	fclose(ptr);
	strcpy(tmp_add, repo_add);
	strcat(tmp_add, "/.neogit/branches/");
	strcat(tmp_add, second_bra);
	strcat(tmp_add, "/commits");
	copy_file_into_array(tmp_add);
	nn2 = 0;
	for (int i = 0; i < size_of_copied_file_array; i++) sscanf(copied_file_array[i], "%d", &arr22[nn2++]);
	int tt1 = nn1 - ind1 - 1;
	int tt2 = nn2;
	int khat = tt1;
	if (tt2 > tt1) khat = tt2;
	...
	printf("| /\n%d\n", arr11[ind2_bad]);
	for (int i = ind2_bad + 1; i < nn1; i++) printf("|\n%d\n", arr11[i]);
	return;
}
...
int main(int argc, char *argv[])
{
	...
	*/
	...
	if (strcmp(argv2[1], "merge") == 0)
	{
		...
		run_merge(argc, argv2);
		return 0;
	}
	if (strcmp(argv2[1], "tree") == 0)
	{
		get_repo_add();
		if (strlen(repo_add) == 0)
		{
